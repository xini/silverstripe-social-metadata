...
class SiteTreeExtension extends Extension
{
    ...
    public function updateMetaComponents(array &$tags)
    {
        ...
        if ($pageSchemaData && $includeSiteSchemaData) {
            ...
            } elseif (isset($pageSchemaData['@graph']) && isset($pageSchemaData['@context'])) { // page array defines graph of one or multiple types
            ...
            } elseif (count($pageSchemaData) > 1) { // page data define more than one type as an array
                // remove the @context from site data
                unset($configSchemaData['@context']);
            ...
            } elseif (count($pageSchemaData) == 1) { // page data defines one type, wrapped in an array.
                // remove the @context from site data
                unset($configSchemaData['@context']);
                // remove @context from page types
